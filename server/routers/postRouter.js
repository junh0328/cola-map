...
postRouter.post('/', auth, async (req, res) => {
  try {
    ...
    if (store) {
      ...
      const is_posted = await Post.countDocuments({ store: store, user: req.user })
      if (is_posted !== 0) {
      ...
      if (req.body.drink !== storeObject.mostPosted) {
        const reqCount = await Post.countDocuments({ store: store, drink: req.body.drink })
        const mostPostedCount = await Post.countDocuments({ store: store, drink: storeObject.mostPosted })
